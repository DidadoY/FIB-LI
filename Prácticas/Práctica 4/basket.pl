...
writeClauses(MaxCost):-
    ...
    alwaysDifferentTeams,
    noTriples,
    noDoublesOnCertainRounds,
    eachRound1TVMatch,
    noHomeOnCertainRounds,
    maxCost(MaxCost),
    true,!.
...
defineHome:- team(T), round(R), findall(match(T,S,R), difTeams(S,T), Lits), expressOr(home(T,R), Lits), fail.
defineHome.


defineDouble :- team(T), round(R), R > 1, PR is R-1,
  writeClause([home(T,R), home(T,PR), double(T, R)]),
  writeClause([-home(T,R), -home(T,PR), double(T, R)]),
  writeClause([-home(T,R), home(T,PR), -double(T, R)]),
  writeClause([home(T,R), -home(T,PR), -double(T, R)]),
  fail.
...
alwaysDifferentTeams:-  team(T1), team(T2), difTeams(T1,T2), findall(match(T1, T2, R), round(R), LitsA),findall(match(T2, T1, R), round(R), LitsH), append(LitsA, LitsH, Lits), exactly(1,Lits), fail.
alwaysDifferentTeams.

noDoublesOnCertainRounds:- round(R), noDoubles(N), member(R, N), findall(double(T,R), team(T), Lits), exactly(0,Lits), fail.
noDoublesOnCertainRounds.


eachRound1TVMatch:- round(R), findall(match(S,T,R), tvMatch(S,T), Lits), atLeast(1, Lits), fail.
eachRound1TVMatch.

noHomeOnCertainRounds:- team(T), findall(match(T,_,R), away(T,R), Lits), exactly(0, Lits), fail.
noHomeOnCertainRounds.
