...
satVariable( ocupada(T,H) ):- task(T), integer(H).
...
eachTaskStartsOnce(MaxHours):- task(T), duration(T,D), MH is MaxHours+1-D,
  findall(start(T,H), between(1,MH,H), Lits), exactly(1,Lits), fail.
eachTaskStartsOnce(_).

defineOcupada(MaxHours):-
  task(T),
  duration(T,D),
  MaxH is MaxHours + 1 - D,
  between(1,MaxH, IniH),
  MinH is IniH + D - 1,
  between(IniH, MinH, PosH),
  writeClause([-start(T, IniH), ocupada(T,PosH)]),
  fail.
defineOcupada(_).
...
limitResources(MaxHours):-
    ...
    findall(ocupada(T,H), (task(T,_,L), member(R,L)), Res),
    atMost(N, Res),
    fail.
limitResources(_).
