...
minimo(C, Num, M, Res):-
	Num1 is Num+1, minimo(C, Num1, M, Res2), distance(C,Num, Km), Km < Res, Res 

lowerBoundOfRemainingCities2([C], LB).
lowerBoundOfRemainingCities2(C, [C2|Cities], LB):-
	distance(C,C2, Km),
	lowerBoundOfRemainingCities2(C, Cities, LB2),
	LB is min(Km, LB2).

lowerBoundOfRemainingCities([C], LB).
lowerBoundOfRemainingCities([C|Cities], LB):- 
	LBC is 100000, LBC2 is 100000, write('llego'), lowerBoundOfRemainingCities2(C,Cities, LBC), lowerBoundOfRemainingCities(Cities, LBC2), LB is min(LBC, LBC2).
...
tsp( Cities, AccumulatedKm, _ ):- bestRouteSoFar(Km,_),
	write(Cities), write('wwww'), write(LBound),
