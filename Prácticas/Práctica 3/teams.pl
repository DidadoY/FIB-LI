...
incompatibleWorkers(W1,W2):- workerScore(W1, S1), workerScore(W2, S2), maxScore(N), S1 + S2 > N.
...
minWorkers:- team(T), minSize(N), findall(wt(W,T), worker(W), Lits), atLeast(N, Lits), fail.
minWorkers.

maxWorkers:- team(T), maxSize(N), findall(wt(W,T), worker(W), Lits), atMost(N, Lits), fail.
maxWorkers.

notIncompatibleWorkers:- worker(W1), worker(W2), W1\=W2, incompatibleWorkers(W1,W2), team(T), writeClause([-wt(W1, T), -wt(W2, T)]), fail.
notIncompatibleWorkers.
