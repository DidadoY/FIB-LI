...
eachEventOneDay:- event(E), eventDays(E, R), findall(ed(E, D), member(D, R), Lits), exactly(1, Lits), fail.
eachEventOneDay.

eachModeratorOneEvent:- event(E), eventModerators(E, R), findall(em(E, M), member(M, R), Lits), exactly(1, Lits), fail.
eachModeratorOneEvent.

maxDays:- maxDaysPerModerator(N), moderator(M), findall(md(M,D), day(D), Lits), atMost(N, Lits), fail.
maxDays.

maxEventsPerDay:- day(D), maxEventsPerDay(N), findall(ed(E, D), event(E), Lits), atMost(N, Lits), fail.
maxEventsPerDay.

%ed(E,D) y em(E,M) -> md(M,D)
%no (ed(E,D) y em(E,M) o md(M,D)
%no (ed(E,D) o no em(E,M) o md(M,D)
juntarClausulas:- day(D), event(E), moderator(M), writeClause([-ed(E,D), -em(E,M), md(M, D)]), fail.
juntarClausulas.
