...
noDoubles:- round(R), noDoubles(N), member(R, N), findall(double(T, R), team(T), Lits), exactly(0, Lits), fail.
noDoubles.

atmostOneTVMatchPerRound:- round(R), findall(match(S,T,R), tvMatch(S-T) , Lits), atMost(1, Lits), fail.
atmostOneTVMatchPerRound.

sixOrSevenHomes:- team(T), findall(home(T, R), round(R), Lits), atLeast(6, Lits), atMost(7, Lits), fail.
sixOrSevenHomes.
